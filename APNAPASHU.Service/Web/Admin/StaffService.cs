...
namespace APNAPASHU.Service.Web.Admin
{
    public class StaffService : BaseService, IStaffService
    {
        ...
        public async Task<JsonModel<List<StaffResponseModel>>> GetAllAsync(FilterDto filterDto)
        {
            ...
            return new JsonModel<List<StaffResponseModel>>(data, ResponseMessages.fetchedSuccessfully, (int)HttpStatusCode.OK);
        }

        public async Task<JsonModel<StaffResponseModel>> GetByIdAsync(int id)
        {
            var data = await _repository.GetByIdAsync(id);
            if (data == null)
            {
                return new JsonModel<StaffResponseModel>(null, ResponseMessages.NotFound, (int)HttpStatusCode.NotFound);
            }
            var encryptionKey = Configuration.GetSection("JwtSettings")["EncryptionKey"] ?? string.Empty;
            ...
            return new JsonModel<StaffResponseModel>(data, ResponseMessages.fetchedSuccessfully, (int)HttpStatusCode.OK);
        }

        public async Task<JsonModel<object>> UpsertAsync(StaffUpsertModel model, int userId)
        {
            ...
            if (!string.IsNullOrWhiteSpace(model.Password))
            {
                ...
                // Store AES encrypted password so it can be decrypted and bound in Edit mode
